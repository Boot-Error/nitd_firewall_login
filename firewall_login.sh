...
flogin() {
	...
	LOGIN_PAGE=$(curl $CURL_FLAGS $HEADERS $FIREWALL_IP/login?)
	...
	RESULT=$(curl $CURL_FLAGS $FIREWALL_IP \
...
flogout() {
	...
	curl $CURL_FLAGS "$FIREWALL_IP/logout?"
}

fkeepalive() {

	curl $CURL_FLAGS "$FIREWALL_IP/keepalive?0000000000000000000"
}
